...
function formatVoteCount(count, direction) {
    const noun = direction === "approve"
...
async function createThreadReply(postId, input, button, listEl, emptyEl, countEl) {
    ...
    try {
        ...
        const nextTotal = Math.max(Number.isFinite(renderedTotal) ? renderedTotal : 0, Number.isFinite(previousTotal) ? previousTotal + 1 : 1);
